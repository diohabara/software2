...
// 課題1] 引数:
//  Itemsetの必要パラメータが記述されたバイナリファイルのファイル名 filename
//  (char*)
// 返り値:
//  Itemset へのポインタ
Itemset *load_itemset(char *filename);
...
// テスト用] 引数: Itemsetの必要パラメータを吐き出すファイルの名前 filename
// (char*) 返り値:
//  なし
void save_itemset(char *filename);
...
double search(int index, const Itemset *list, double capacity,
  ...
  flags[index] = 1;
  const double v1 =
      search(index + 1, list, capacity, flags, sum_v + list->item[index].value,
             sum_w + list->item[index].weight);
